  ...
  <base-layout>
    ...
    <view class="myPage" :class="[btn === false ? 'big-box':'big-box-night']">
      <view v-for="(item,index) in list" class="flex-j-a list-box" @click="toPage(item.url)">
        <view class="align">
          <image :src="item.img" mode="" style="width: 40rpx;height: 40rpx;margin-right: 32rpx;"
                 class="list-picture"></image>
          <view class="tex">{{ item.title }}</view>
        </view>
        <u-badge v-if="index == 3 && info.opinion == 1" class="badge-num2" :is-dot="true" type="error"
                 size="mini"></u-badge>
