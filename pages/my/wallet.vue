...
<!-- 					<image :src="item.select" v-if="currentCategory==item.category" mode=""></image>
			...
			<view>
				<view class="flexs" v-for="(item,index1) in walletList" :key="index1" > 
					...
					<view class="items flexs" @click="goTo('/pages/wallet/walletDetail',item.id)">
						<view class="tip"></view>
						<view style="margin-left: 30rpx;">
							<view><text class="color-f8" v-if="currentText == 'DLMU' && index1 == 0">{{$t('home.txt148',['初始'])}}</text>{{"Telegram Pocket"}}</view>
			...
			<view class="parent">
				...
				<view class="otherMoney">
					<view class="commonfont" style="font-size: 30rpx;border-bottom: 1rpx solid #E6E6E6;"
						@click="goTo('/pages/index/addcreate?type=1','')">{{$t('ibinz.msg12',["创建钱包"])}}</view>
					<view class="commonfont" style="font-size: 30rpx;border-bottom: 8rpx solid #E6E6E6;"
						@click="goTo('/pages/index/addcreate?type=2','')">{{$t('ibinz.msg13',["导入钱包"])}}</view>
...
<script>
	...
	export default class Idnex extends Vue {
		...
		async onShow() {
			...
			await this.getList();
			await this.init();
			await this.getwalletList(uni.getStorageSync('currentWallet').category);
