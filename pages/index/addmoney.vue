	...
	<base-layout>
		<view>
			...
			<view style="padding: 0 30rpx;">
				...
				<base-button v-if="select" ref="button" class="nosubmit" @submit="submit()" :title="btnText" />
				<button v-else class="nosubmit"><view class="tex">{{$t('home.txt22',['创建钱包'])}}</view></button>
	...
	export default {
		...
		methods: {
            ...
            saveConfig(data.address,data.keystore)
						...
						// 波场钱包-私钥验证
						// console.log(data.mnemonics, 'youmeiy');
						// if (this.category == 3 || this.category == 5) {
						// 	uni.setStorageSync("privateKey", data.private_key)
						// } else {
						// 	// 助记词
						// 	uni.setStorageSync('mnemonics', data.mnemonics)
						// 	uni.setStorageSync('mnemonicsType', '1') //mnemonicsType 1 调用创建钱包助记词验证接口
						// }
						this.$refs.button.hideLoading()
						// uni.navigateTo({
