    ...
    <view :class="[btn === false?'box':'box-night']">
      ...
      <view class="usermoney">
        ...
        <view style="align-items: center;display: flex;margin-top: 60rpx;">
          ...
          <view :class="[btn === false?'zz-text':'zz-text-night']">
            {{$t('home.txt121',['转账历史'])}}
          </view>
        </view>
        <noData v-if="dataList.length<1"></noData>
        <view v-else :class="[btn === false?'list-items':'list-items-night']" v-for="(item,index) in dataList" @click="clickDetails(item)">
          <view class="list-box">
            <!-- 转出 -->
